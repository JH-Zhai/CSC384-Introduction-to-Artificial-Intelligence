    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                for i in range(4):
                    process.append(
                        (nextx, nexty) == self.corners[i] or state[i + 1])
                newState = ((nextx, nexty), process[0], process[1], process[2], process[3])
...
def cornersHeuristic(state, problem):
    ...
    for i in range(1, 5):
        if not state[i]:
            counter += 1
            re_var += abs(pos[0] - corners[i-1][0]) + abs(pos[1] - corners[i-1][1])
    if counter != 0:
        re_var = re_var/counter
    return re_var*2.25
...
def foodHeuristic(state, problem):
    ...
    post1 = foodGrid.asList()[0]
    post2 = foodGrid.asList()[1]
    dis = util.manhattanDistance(post1, post2)
    for i in foodGrid.asList():
        for j in foodGrid.asList():
            if util.manhattanDistance(i,j) > dis:
                post1 = i
                post2 = j
                dis = util.manhattanDistance(i,j)
    return min((util.manhattanDistance(position, post1)),(util.manhattanDistance(position,post2))) + util.manhattanDistance(post1,post2)
