...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        ghostPosts = [g.argMax() for g in livingGhostPositionDistributions]
        targetPost = None
        targetDis = 9999999
        for p in ghostPosts:
            if self.distancer.getDistance(pacmanPosition, p) < targetDis:
                targetDis = self.distancer.getDistance(pacmanPosition, p)
                targetPost = p
        action = None
        afterActDis = 9999999
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            if self.distancer.getDistance(successorPosition, targetPost) < afterActDis:
                action = a
                afterActDis = self.distancer.getDistance(successorPosition, targetPost)
        return action
