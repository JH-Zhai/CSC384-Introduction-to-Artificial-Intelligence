...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        if self.numUnassigned() > 0: return True
        counter = 0
        for var in self.scope():
            if var.getValue() in self._required: counter += 1
        return counter <= self._ub and counter >= self._lb
    ...
    def hasSupport(self, var, val):
        ...
        for v in allVars:
            if v.isAssigned():
                if v.getValue() in self._required: setCounter += 1
            else:
                has = False
                all = True
                for l in v.curDomain():
                    if l in self._required:
                        has = True
                    if l not in self._required:
                        all = False
                if all:
                    setCounter += 1
                else:
                    if has:
                        protCounter += 1
        if val in self._required: setCounter += 1
        if setCounter <= self._ub and setCounter >= self._lb:
            return True
        if setCounter > self._ub:
            return False
        if setCounter + protCounter >= self._lb:
            return True
