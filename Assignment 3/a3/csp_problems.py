...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint("alldiff1", row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint("alldiff2", scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint("alldiff3", scope))
...
def solve_planes(planes_problem: PlaneProblem, algo, allsolns,
    ...
    maintFlights = planes_problem.maintenance_flights
    mainFreq = planes_problem.min_maintenance_frequency
    follow = planes_problem.can_follow
    vars = []
    varsByPlane = []
    cons = []
    lenOfEachPlaneVars = []
    # print(f"how many planes {len(planes)}")
    for p in planes:
        canFlights = planes_problem.can_fly(p)
        # print(f"how many flights available line 347 {len(canFlights)}")
        lenOfEachPlaneVars.append(len(canFlights))
        canStarts = planes_problem.can_start(p)
        # print(f"how many flights available line 350 {len(canFlights)}")
        currVars = []
        firstDomain = [value for value in canStarts if value in canFlights]
        firstDomain.append(0)
        # print(f"how many flights available line 354 {len(canFlights)}")
        restDomain = canFlights.copy()
        restDomain.append(0)
        # print(f"how many flights available before for loop in line 357 {len(canFlights)}")
        for i in range(len(canFlights)):
            if i == 0:
                var = Variable(p, firstDomain)
                vars.append(var)
                currVars.append(var)
            else:
                var = Variable(p, restDomain)
                vars.append(var)
                currVars.append(var)
        varsByPlane.append(currVars)
        # print(f"length of vars after each generation {len(vars)}")
    # print(f"length of vars after enerated {len(vars)}")

    # Constraint that each flight once
    for f in flights:
        con1 = NValuesConstraint(f, vars, [f], 1, 1 )
        cons.append(con1)

    # Constraint that first flight from start && flights in geo order
    satisfyingAssignments = []
    for t in follow:
        satisfyingAssignments.append(list(t))
    for f in flights:
        satisfyingAssignments.append([f, 0])
    satisfyingAssignments.append([0, 0])
    for plVars in varsByPlane:
        for i in range(len(plVars)):
            if not i == (len(plVars) - 1):
                con2 = TableConstraint(plVars[i].name(), [plVars[i], plVars[i+1]], satisfyingAssignments)
                cons.append(con2)

    # Constraint that maintain
    required = maintFlights.copy()
    required.append(0)
    for plVars in varsByPlane:
        if len(plVars) > mainFreq:
            for i in range(0, len(plVars) - mainFreq + 1):
                scope = []
                for j in range(i, i + mainFreq):
                    scope.append(plVars[j])
                con3 = NValuesConstraint("name", scope, required, 1, mainFreq)
                cons.append(con3)

    # print(f"length of var before CSP {len(vars)}")

    csp = CSP("name", vars, cons) #set to to your CSP
    #invoke search with the passed parameters
    solutions, num_nodes = bt_search(algo, csp, variableHeuristic, allsolns, trace)
    # print(f" length of solution {len(solutions)}")
    # print(f" how many planes {len(solutions[0])}")
    ...
    for s in solutions:
        ...
        thisSolution = []
        for i in s:
            lstVars.append(list(i))

        for i in lenOfEachPlaneVars:
            thisPlane = [lstVars[0][0].name()]
            for j in range(i):
                f = lstVars.pop(0)[1]
                if f != 0:
                    thisPlane.append(f)
            thisSolution.append(thisPlane)
        re_val.append(thisSolution)
